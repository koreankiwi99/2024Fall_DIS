...
class OurBM25:
  ...
  def search(self,
    ...
    if self.save_form == 'fast': #loading slow, fast retrieval
      ...
      scores = np.array([sum([doc[idx] for idx in word_indices if idx in doc]) for doc in self.document_score])
    
    top_doc_k = np.argpartition(-scores, k)
    top_doc_k = top_doc_k[:k]

    if return_score:
      sorted_score = [scores[idx] for idx in top_doc_k]

      if self.corpus_id:
        return (sorted_score, [self.corpus_id[idx] for idx in top_doc_k])

      else:
        return (sorted_score, top_doc_k)
